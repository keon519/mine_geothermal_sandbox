"""One dataclass holding every knob of the sandbox.

The four variables you said you want to sweep are at the top:
    flow_m3_h, T_in_C, shaft_length_m / shaft_diameter_m, rock
Everything below those is a numerical or "how the physics is closed" knob.
"""
...
class SandboxConfig:
    ...
    # ---- how the shaft interacts with the rock ---------------------------
    T_rock_C: float = 13.0            # undisturbed rock / groundwater temperature [degC]
    shaft_porosity: float = 1.0       # 1.0 = open flooded drift; <1 if partially filled with rubble
    h_film_W_m2K: float = 800.0       # convective film coefficient water -> wall [W/m^2/K]
    rock_outer_radius_m: float = 60.0 # where rock temperature is pinned at T_rock_C [m]

    # ---- discretisation --------------------------------------------------
    n_x: int = 50                     # cells along the shaft
    n_r: int = 14                     # radial rock shells around the shaft
    sim_days: float = 365.0           # total simulated time
    dt_days: float = 0.5              # heat-transport time step
    adv_scheme: str = "UPSTREAM"      # "UPSTREAM" (robust at any flow) or "TVD" (sharper fronts, may fail at high Courant)

    # ---- TESPy surface loop (only used by the coupled runner) ------------
    ...
    T_max_C: float = 95.0             # stop a coupled run if the loop water gets this hot (runaway / boiling)
